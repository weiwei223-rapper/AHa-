...
def chat_with_ai(payload: ChatRequest, db: Session = Depends(database.get_db)):
    ...
    try:
        reply = ai_analyzer.generate_chat_reply(user_message, [{"role": item.role, "content": item.content} for item in
                                                               payload.history], video_context)
...
def recharge_user(user_id: int, payload: RechargeRequest, db: Session = Depends(database.get_db)):
    ...
    if user is None:
        raise HTTPException(status_code=404, detail="User not found")
    record = models.RechargeRecord(user_id=user.id, date=datetime.utcnow().strftime("%Y/%m/%d"),
                                   order_id=f"A{datetime.utcnow():%Y%m%d%H%M%S}", amount=payload.price,
                                   points=payload.points, plan_content=payload.plan_content,
                                   payment_method=payload.payment_method, plan_id=payload.plan_id)
